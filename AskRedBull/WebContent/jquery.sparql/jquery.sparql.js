 ...
 * jQuery SPARQL
 * 
 * Provides an idiomatic jQuery-like interface to SPARQL endpoints. Queries are
 * built through method-chaining, before being compiled into a string query
 * which can be sent to the endpoint.
 * 
 * $.sparql("http://www.example.com/sparql/").prefix("foaf","http://xmlns.com/0.1/foaf/")
 * .select() .where("?p", "a", "foaf:Person") .where("foaf:name", "?name")
 * .where("foaf:homepage", "?page") .orderby("?name") .distinct()
 * .execute(cbfunc);
 */
(function($) {
	...
	Query.prototype.execute = function(callback) {
			...
			if(data.results){
				...
				if ($.isArray(results)) {
					for ( var r in results) {
						...
						if (!$.isEmptyObject(cleaned_obj))
							cleaned_results.push(cleaned_obj);
						else
							cleaned_results.push(result);
					}
				} else {
					var cleaned_obj = {};
					for ( var k in results) {
						cleaned_obj[k] = _clean(results[k]);
					}
					if (!$.isEmptyObject(cleaned_obj))
						cleaned_results.push(cleaned_obj);
					else
						cleaned_results.push(result);
					// cleaned_results.push(_clean(results));
				}
				console.log("cleaned_results: "
						+ JSON.stringify(cleaned_results));
		...
		if (queryString == "")
			queryString = this.serialiseQuery();
		if (method == "GET") {
//			var yqlQuery = 'use "http://triplr.org/sparyql/sparql.xml" as sparql; select * from sparql where query="'
//					+ queryString + '" and service="' + endpoint + '"';
//			$.yql(yqlQuery, _preproc);
			$.ajax({
				url : endpoint+"?default-graph-uri="+def_graph_uri+"&format=json&query="+$.URLEncode(queryString),
				dataType : "jsonp",
				success : _preproc
			});
	...
	Query.prototype.serialiseQuery = function() {
		...
		if (this.queryType == "SELECT" && this.variables.length == 0) {
			queryString.push("*"); // No variables is seen as an implicit
			// projection over ALL variables
		} else {
	...
	Query.prototype.where = function(subj, prop, obj) {
		if (!obj && !prop) {
			// We're in a subj-prop repeating section, use previous subj and
			// prop
